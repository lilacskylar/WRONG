                    ...
                    if (newValue.length() > 0)
                    {
                        ...
                        catch (Exception e)
                        {
                            ...
                            // TODO: if the value is not a possible value, catch the exception and show an alert
                             if (!board.isLegal(r, c, Integer.parseInt(newValue))){
    ...
    private MenuBar createMenuBar(Stage primaryStage)
    {
        ...
        addMenuItem(hintMenu, "Show hint", () -> {
            ...
            for (int row = 0; row < SIZE; row++)
            {
                ...
                {
                    TextField textField = textFields[row][col];
                    String possible = board.getPossibleValues(row, col).toString();
                    if (possible.length() == 1){
                        if (textField.getStyleClass().add("text-field-highlighted"))
                        updateBoard();
                    }
            }
        }
